...
const Products = () => {
  ...
  // Backend pagination data for category mode
  ...
  // Reload when pagination changes in category mode
  useEffect(() => {
    if (isCategoryMode) {
      loadProductsByCategory()
    }
  }, [currentPage, productsPerPage, isCategoryMode])
  ...
  // Client-side pagination for overview mode, backend pagination for category mode
  const paginationData = useMemo(() => {
    ...
    // Fallback for client-side pagination (shouldn't be used in category mode anymore)
  ...
  }, [currentPage, productsPerPage, products, isCategoryMode, paginationInfo])
  ...
  // Memoize expensive calculations for overview mode
  ...
  }, [categories, products, isCategoryMode])

  const recentProducts = useMemo(() => {
    if (isCategoryMode) return []
    return products.slice(0, 8)
  }, [products, isCategoryMode])

  // Get products to display based on mode
  const displayProducts = useMemo(() => {
    if (isCategoryMode) {
      return products // In category mode, products already contain only the current page
    }
    return recentProducts
  }, [isCategoryMode, products, recentProducts])
  ...
  return (
    <div className="w-100">
      ...
      {/* Category Filter Tabs - Only show in overview mode */}
      {!isCategoryMode && (
      ...
      <div className="w-100">
          {products.length === 0 ? (
            <div className="text-center py-5">
              ...
              <button className="btn btn-primary">
                <i className="bi bi-plus-lg me-2"></i>
                {isCategoryMode ? 'Add First Product' : 'Add First Product'}
              </button>
            </div>
          ) : (
            <>
            <div className="table-responsive w-100">
              ...
              {/* Pagination - Always show in category mode */}
